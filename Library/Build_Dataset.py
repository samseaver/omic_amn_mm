...
def get_B(model, S, medium, verbose=False):
    ...
    for rid in medium:
        ...
        r = model.reactions[k]
        # print(r.products)
        p = r.products[0] if r.products else r.reactants[0]# medium reactions have only one product
...
def get_matrices_LP(model, mediumbound, X, S, Pin, medium, objective, verbose=False):
    ...
    if X.shape[1] <= len(medium):
        inputs = np.float32(X)
    else:
        inputs = np.float32(X[:, :len(medium)])
    ...
    if mediumbound == 'UB':
        ...
        for rid in medium:
            ...
            # Whereas   is the stoechiometric matrix for the
