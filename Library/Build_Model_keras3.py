...
def generate_loss_model(parameter, Vin, Vbf):
    ...
    out_shape = (None, )
    out = tf.keras.layers.Lambda(compute_loss_ops, output_shape=out_shape) \
...
class CustomLoss(Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        self.Vin = ops.expand_dims(self.Vin, axis=0)
        self.Vbf = ops.expand_dims(self.Vbf, axis=0)
        y_pred   = ops.expand_dims(y_pred, axis=0)
        y_true   = ops.expand_dims(y_true, axis=0)
        y_true   = ops.convert_to_tensor(y_true)

        loss = compute_loss_ops(y_true, y_pred, self.Vbf, self.Vin, self.S, self.Pin, self.Pout)
...
class TensorLayer(keras.layers.Layer):
    def call(self, y_true, y_pred, Vbf, Vin, S, Pin, Pout):
        return compute_loss_ops(y_true, y_pred, Vbf, Vin, S, Pin, Pout)
...
def biochem_loss(parameter, paramVin, paramVbf):
    ...
    def loss(y_true, y_pred):
        ...
        # y_true = ops.expand_dims(y_true, axis=0)
        # y_pred = ops.expand_dims(y_pred, axis=0)
        # Vbf = ops.expand_dims(Vbf, axis=0)
        # Vin = ops.expand_dims(Vin, axis=0)
        # S = ops.expand_dims(S, axis=0)
        # Pout = ops.expand_dims(Pout, axis=0)
        # Pin = ops.expand_dims(Pin, axis=0)
...
def Loss_Vout(V, Pout, Vout, gradient=False):
    ...
    Loss = ops.matmul(V, tf.transpose(Pout)) - Vout
    Loss_norm = ops.norm(Loss, axis=1, keepdims=True)/Pout.shape[0] # rescaled
    if gradient:
        dLoss = ops.matmul(Loss, Pout) # derivate
...
def Loss_Vout_obj(V, parameter=None, gradient=False):
    ...
    # dLoss = ∂([Pout.V-Vout]^2)/∂V = Pout^T (Pout.V - Vout)
    # print("in Loss_Vout: ", V.shape, Pout.shape, Vout.shape)
    Pout = ops.convert_to_tensor(np.float32(parameter.objPout))
    Vout = ops.convert_to_tensor(np.float32(parameter.objY))
    # print(Pout, Vout, V)
...
def Loss_Vout_constraint(V, Pout, Vout, gradient=False):
    ...
    if gradient:
        ...
        # dLoss = dLoss / (Pout.shape[0] * Pout.shape[0])  # rescaling
        dLoss = ops.divide_no_nan(Loss, Loss) # derivate: Hadamard div.
        dLoss = ops.multiply(Loss, dLoss) # !!!
        dLoss = ops.matmul(dLoss, Pout) # resizing
        dLoss = dLoss / (Pout.shape[0] * Pout.shape[0])   # rescaling
        # dLoss = 2 * dLoss
    else:
        dLoss =  0 * V
...
def Loss_Vin(V, Pin, Vin, bound='UB', gradient=False):
    ...
    if not tf.is_tensor(Pin):
        Pin  = ops.convert_to_tensor(np.float32(Pin))

    Loss = ops.matmul(V, tf.transpose(Pin)) - Vin
    # tf.cast(tf.multiply(Vin, parameter.scaler), tf.float32)
    Loss = tf.keras.activations.relu(Loss) if bound == 'UB' else Loss
    Loss_norm = ops.norm(Loss, axis=1, keepdims=True)/Pin.shape[0] # rescaled
    if gradient:
        dLoss = ops.divide_no_nan(Loss, Loss) # derivate: Hadamard div.
        dLoss = ops.multiply(Loss, dLoss) # !!!
        dLoss = ops.matmul(dLoss, Pin) # resizing
...
def Loss_SV(V, S, gradient=False):
    ...
    if not tf.is_tensor(S):
        S  = ops.convert_to_tensor(np.float32(S))
    Loss = ops.matmul(V, tf.transpose(S))
    Loss_norm = ops.norm(Loss, axis=1, keepdims=True)/S.shape[0] # rescaled
    if gradient:
        dLoss = ops.matmul(Loss, S) # derivate
...
def Loss_Vpos(V, Vlb, gradient=False):
    ...
    Loss_norm = ops.norm(Loss, axis=1, keepdims=True)/V.shape[1] # rescaled
    if gradient:
        dLoss = - ops.divide_no_nan(Loss, Loss) # derivate: Hadamard div.
        dLoss = ops.multiply(Loss, dLoss) # !!!
...
def Loss_constraint(V, Vin, Vlb, parameter, gradient=False):
    ...
    L3 = ops.square(L3)
    L4 = ops.square(L4)
    L = tf.reduce_sum(tf.concat([L2, L3, L4], axis=1), axis=1)
    # divide by 3
    L = ops.divide_no_nan(L, tf.constant(3.0, dtype=tf.float32))
...
def Loss_all5(V, Vin, Vout, Vlb, parameter, gradient=False, p_sv=1):
    ...
    L = ops.reduce_sum(tf.concat([L1, L2, L3, L4, L5], axis=1), axis=1)
    # divide by 4

    num_const = 5.0
    L = ops.divide_no_nan(L, tf.constant(num_const, dtype=tf.float32))
    return L, dL1+dL2+dL3+dL4+dL5

def Loss_all(V, Vin, Vout, Vlb, parameter, gradient=False, wt=False, p_sv=1):
    # mean square sum of L1, L2, L3, L4, L5
    ...
    # L = ops.reduce_sum(tf.concat([L1, L2, L3, L4, L5], axis=1), axis=1)
    L = tf.reduce_sum(tf.concat([L1, L2, L3, L4], axis=1), axis=1)
    # divide by 4
    num_const = 4.0
    L = ops.divide_no_nan(L, tf.constant(num_const, dtype=tf.float32))
    # if wt:
...
def output_AMN(V, Vin, V0, Vlb, parameter, verbose=False):
    ...
    Pout     = tf.convert_to_tensor(np.float32(parameter.Pout))
    PoutV    = ops.matmul(V, tf.transpose(Pout))#, b_is_sparse=True)
    ...
    def call(self, inputs, states):
        ...
        if self.mediumbound == 'UB':
            if self.hidden_dim > 0:
                VH = ops.dot(inputs, self.wh_V)
                V0 = ops.dot(VH, self.wi_V) + self.bi_V
            else:
                V0 = ops.dot(inputs, self.wi_V) + self.bi_V
        else:
            V0 = inputs # EB case
...
def AMN_Wt(parameter, verbose=False):
    ...
    loss, metrics = custom_loss, None # biochem_loss(parameter, Vin, Vbf), None
        ...
        if self.model_type == 'AMN_Wt':
            self.model = load_model(filemodel,
                                    custom_objects={'RNNCell':RNNCell,
                                                    ...
                                                    # , 'loss': biochem_loss(parameter, Vin, Vbf)},
                                    compile=False)
